	...
	namespace core
	{
		...
		public:
			...
			*/
			...
			// CUSTOM ADDED: Quaternion to represent a look rotation towards a vector based on the direction
			quaternion& lookRotation(const vector3df& from, const vector3df& to);
		...
		*/
		...
		inline core::quaternion& quaternion::lookRotation(const vector3df& lookAt, const vector3df& upDirection)
		{
			vector3df forward = lookAt;
			vector3df up = upDirection;

			forward.normalize();
			vector3df v = forward.crossProduct(up);
			v.normalize();
			up = v.crossProduct(forward);
			...
			w = (float)sqrtf(1.0f + right.X + up.Y + forward.Z) * 0.5f;
